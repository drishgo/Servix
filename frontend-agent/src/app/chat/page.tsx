...
import { Send, Bot, User, Sparkles, Paperclip, FileText, X, ChevronLeft, ChevronRight, Plus, Search, Settings, Info } from "lucide-react";
...
export default function ChatInterface() {
  ...
  return (
    <main className="h-screen w-full flex flex-row bg-[#000000] overflow-hidden font-sans text-white relative">
      ...
      <aside
        className={`${isSidebarOpen ? "w-[85vw] md:w-[30%] border-r-4 p-6 md:p-8" : "w-0 border-r-0 p-0"
          } bg-[#E2FF00] text-black flex flex-col justify-between border-black absolute md:relative h-full z-40 transition-all duration-300 ease-in-out`}
      >
        ...
        <div className={`relative z-10 transition-opacity duration-200 whitespace-nowrap overflow-hidden ${isSidebarOpen ? "opacity-100 delay-100" : "opacity-0"}`}>
          ...
          <div className="space-y-4">
            ...
            {/* Dummy Search Bar */}
            <div className={`relative transition-all duration-300 ease-in-out ${isSidebarOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4 pointer-events-none"}`}>
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#E2FF00] z-10" />
              <Input
                type="text"
                placeholder="SEARCH ARCHIVES..."
                className="w-full bg-black border-2 border-black text-[#E2FF00] placeholder:text-[#E2FF00]/50 placeholder:font-bold placeholder:tracking-widest rounded-none pl-10 h-10 shadow-[2px_2px_0px_#000000] focus-visible:ring-0 focus-visible:ring-offset-0 font-mono text-sm"
              />
            </div>
          </div>
        </div>
      ...
      <button
        ...
        className={`absolute top-1/2 -translate-y-1/2 z-50 flex items-center justify-center w-6 h-12 bg-black border-2 border-white text-white hover:bg-[#E2FF00] hover:text-black hover:border-black transition-all duration-300 ease-in-out focus:outline-none ${isSidebarOpen ? "left-[calc(85vw-12px)] md:left-[calc(30%-12px)]" : "left-[-2px]"
          }`}
      ...
      <section className="flex-1 w-full bg-[#000000] relative overflow-hidden flex flex-col transition-all duration-300 ease-in-out">
        {/* Sticky Top Navbar */}
        <header className="sticky top-0 z-20 w-full bg-[#E2FF00] border-b-4 border-black p-4 flex justify-between items-center shrink-0 transition-colors">
          {/* Left Side: Settings and About */}
          <div className="flex items-center gap-2 sm:gap-4 z-10">
            <Button
              size="icon"
              className="bg-black text-[#E2FF00] hover:bg-white hover:text-black border-2 border-black rounded-none shadow-[2px_2px_0px_#000000] transition-all shrink-0"
              title="Settings"
            >
          ...
          {/* Center: Servix Badging / Conversation Title Placeholder */}
          <div className="absolute left-1/2 -translate-x-1/2 pointer-events-none flex items-center justify-center w-[50%] sm:w-auto text-center z-0">
            <span className="font-black text-xl sm:text-2xl tracking-tighter uppercase italic text-black truncate">
              SERVIX
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4 justify-end z-10">
            <Button
              className="bg-black text-[#E2FF00] hover:bg-white hover:text-black border-2 border-black rounded-none shadow-[2px_2px_0px_#000000] font-bold uppercase tracking-widest gap-2 transition-all"
            >
